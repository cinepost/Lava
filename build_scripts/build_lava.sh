...
#    libminizip-dev g++-multilib libzzip-dev libtiff-dev libgif-dev libopencolorio-dev libraw-dev libwebp-dev python3.7-dev libpython3.7-dev rapidjson-dev liblz4-dev
...
#cd $LAVA_3RDPARTY_SOURCE_DIR/tbb
#export INCDIR=$LAVA_3RDPARTY_INC_INSTALL_DIR
#export INSTALLDIR=$LAVA_3RDPARTY_LIB_INSTALL_DIR
#make -j$CPU_CORES

cd $LAVA_3RDPARTY_SOURCE_DIR/c-blosc2
...
#cd $LAVA_3RDPARTY_SOURCE_DIR/oneTBB
#mkdir -p build && cd build
#cmake .. -DCMAKE_BUILD_TYPE=RELEASE -DCMAKE_INSTALL_PREFIX=${LAVA_3RDPARTY_INSTALL_DIR} && make -j$CPU_CORES install

cd $LAVA_3RDPARTY_SOURCE_DIR/openvdb
mkdir -p build && cd build
cmake .. -DCMAKE_BUILD_TYPE=RELEASE -DCMAKE_INSTALL_PREFIX=${LAVA_3RDPARTY_INSTALL_DIR} \
    && make -j$CPU_CORES install

cd $LAVA_3RDPARTY_SOURCE_DIR/openvdb/nanovdb
mkdir -p build && cd build
cmake .. -DNANOVDB_USE_CUDA=OFF -DNANOVDB_USE_OPTIX=OFF \
	-DCMAKE_BUILD_TYPE=RELEASE -DCMAKE_INSTALL_PREFIX=${LAVA_3RDPARTY_INSTALL_DIR}
