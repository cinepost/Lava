...
#define GFX_FORMAT(x) \
	...
	x(R16G16B16_TYPELESS, 6, 1) \
	x(R16G16_TYPELESS, 4, 1) \
	x(R16_TYPELESS, 2, 1) \
	\
	x(R8G8B8A8_TYPELESS, 4, 1) \
	x(R8G8B8_TYPELESS, 3, 1) \
	x(R8G8_TYPELESS, 2, 1) \
	x(R8_TYPELESS, 1, 1) \
...
public:
	...
	struct Desc: public DescBase {
		...
		bool        sparse = false;     ///< Sprase texture resource required
	};
...
public:
	...
	virtual SLANG_NO_THROW Result SLANG_MCALL
		bindPipelineWithRootObject(IPipelineState* state, IShaderObject* rootObject) = 0;
	virtual SLANG_NO_THROW Result SLANG_MCALL dispatchCompute(uint32_t x, uint32_t y, uint32_t z) = 0;
...
class IDevice: public ISlangUnknown {
	public:
		...
		inline SLANG_NO_THROW ComPtr<IBufferResource> createBufferResource(const IBufferResource::Desc& desc, const void* initData = nullptr) {
		...
		inline ComPtr<IResourceView> createBufferView( IBufferResource* buffer, IBufferResource* counterBuffer, IResourceView::Desc const& desc) {
			ComPtr<IResourceView> view;
