...
PipelineStateImpl::PipelineStateImpl(DeviceImpl* pDevice) {
	...
	mpPipelineCache = PipelineCache::create(pDevice);
}
...
void PipelineStateImpl::destroy() {
	return; 
	if (m_pipeline != VK_NULL_HANDLE) {
		m_device->m_api.vkDestroyPipeline(m_device->m_api.m_device, m_pipeline, nullptr);
	}

	mpPipelineCache.reset();
}
...
Result PipelineStateImpl::createVKGraphicsPipelineState() {
	...
	multisampling.rasterizationSamples = (forcedSampleCount == 0) ? framebufferLayoutImpl->m_sampleCount : VulkanUtil::translateSampleCount(forcedSampleCount);
	...
	auto targetCount = GfxCount(Math::Min(framebufferLayoutImpl->m_renderTargetCount, (uint32_t)blendDesc.targetCount));
	...
	if (targetCount == 0) {
		...
		for (GfxIndex i = 0; i < targetCount; ++i) {
			...
			vkBlendDesc.srcColorBlendFactor = VulkanUtil::translateBlendFactor(gfxBlendDesc.color.srcFactor);
			vkBlendDesc.dstColorBlendFactor = VulkanUtil::translateBlendFactor(gfxBlendDesc.color.dstFactor);
			vkBlendDesc.colorBlendOp = VulkanUtil::translateBlendOp(gfxBlendDesc.color.op);
			vkBlendDesc.srcAlphaBlendFactor = VulkanUtil::translateBlendFactor(gfxBlendDesc.alpha.srcFactor);
			vkBlendDesc.dstAlphaBlendFactor = VulkanUtil::translateBlendFactor(gfxBlendDesc.alpha.dstFactor);
	...
	SLANG_VK_CHECK(m_device->m_api.vkCreateGraphicsPipelines(m_device->m_device, pipelineCache, 1, &pipelineInfo, nullptr, &m_pipeline));

	//Falcor::ThreadPool& pool = Falcor::ThreadPool::instance();
  //mResult = pool.submit([this, pipelineCache, pipelineInfo]{
  //	return m_device->m_api.vkCreateGraphicsPipelines(m_device->m_device, pipelineCache, 1, &pipelineInfo, nullptr, &m_pipeline);
  //});
...
Result PipelineStateImpl::createVKComputePipelineState() {
	...
	VkPipelineCache pipelineCache = mpPipelineCache->getPipelineCache();
	...
	SLANG_VK_CHECK(m_device->m_api.vkCreateComputePipelines(m_device->m_device, pipelineCache, 1, &computePipelineInfo, nullptr, &m_pipeline));
	
	//Falcor::ThreadPool& pool = Falcor::ThreadPool::instance();
  //mResult = pool.submit([this, pipelineCache, computePipelineInfo]{
  //	return m_device->m_api.vkCreateComputePipelines(m_device->m_device, pipelineCache, 1, &computePipelineInfo, nullptr, &m_pipeline);
  //});
...
Result PipelineStateImpl::ensureAPIPipelineStateCreated() {
	...
	if(getPipeline()) return SLANG_OK;
...
VkPipeline PipelineStateImpl::getPipeline() {
	if(mResult.valid()) {
		mResult.wait();
	}
	return m_pipeline;
...
uint32_t RayTracingPipelineStateImpl::findEntryPointIndexByName(const Dictionary<String, Index>& entryPointNameToIndex, const char* name) {
	if (!name) return VK_SHADER_UNUSED_KHR;

	auto indexPtr = entryPointNameToIndex.TryGetValue(String(name));
	if (indexPtr) return (uint32_t)*indexPtr;
...
Result RayTracingPipelineStateImpl::createVKRayTracingPipelineState() {
	...
	VkPipelineCache pipelineCache = VK_NULL_HANDLE;
	SLANG_VK_CHECK(m_device->m_api.vkCreateRayTracingPipelinesKHR( m_device->m_device, VK_NULL_HANDLE, pipelineCache, 1, &raytracingPipelineInfo, nullptr, &m_pipeline));
	shaderGroupCount = shaderGroupInfos.getCount();

	//Falcor::ThreadPool& pool = Falcor::ThreadPool::instance();
  //mResult = pool.submit([this, pipelineCache, &raytracingPipelineInfo]{
  //	return VK_SUCCESS;
  //});
