...
Falcor::Dictionary AOVPlane::getMetaData() const { 
	if(mMetaDataCallbacks.size() == 0 && mMetaDataRenderPasses.size() == 0) return mMetaData;

	auto resultMetaData = mMetaData;
	for(auto& func: mMetaDataCallbacks) {
		for (const auto& [key, value] : func()) {
			resultMetaData[key] = value;
		}
	}

	for(auto& pPass: mMetaDataRenderPasses) {
		if(pPass) {
			for (const auto& [key, value] : pPass->getMetaData()) {
				resultMetaData[key] = value;
			}
		}
	}
...
void AOVPlane::addMetaData(const Falcor::Dictionary& meta_data) {
