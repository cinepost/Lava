...
bool DisplayOIIO::openImage(const std::string& image_name, uint width, uint height, Falcor::ResourceFormat format, uint &imageHandle, 
...
bool DisplayOIIO::openImage(const std::string& image_name, uint width, uint height, const std::vector<Channel>& channels, uint &imageHandle, 
	...
	if(pExistingImageData) {
		...
		LLOG_TRC << "imData.supportsRandomAccess " << (imData.supportsRandomAccess ? "true" : "false");
		LLOG_TRC << "imData.supportsTiles " << (imData.supportsTiles ? "true" : "false");
		LLOG_TRC << "imData.supportsMultiImage " << (imData.supportsMultiImage ? "true" : "false");
		LLOG_TRC << "imData.supportsAppendSubImage " << (imData.supportsAppendSubImage ? "true" : "false");
		LLOG_TRC << "imData.supportsPerChannelFormats " << (imData.supportsPerChannelFormats ? "true" : "false");
		LLOG_TRC << "imData.supportsRectangles " << (imData.supportsRectangles ? "true" : "false");
	...
	// Write metadata
	if(pMetaData) {
		for (const auto& [key, value] : *pMetaData) {
	...
	// Proper image name attribute
	std::string nameAttr;
	for(const auto& channel: channels) {
		const std::string token = channel.name.substr(0, channel.name.find("."));
		if (nameAttr == "" && token != "") nameAttr = token;
		else if(nameAttr != "" && token != nameAttr) {
			nameAttr = "";
			break;
		}
	}
	if(nameAttr != "") spec.attribute("name", nameAttr);
...
bool DisplayOIIO::closeImage(uint imageHandle) {
	...
	LLOG_DBG << "Closing display \"" << to_string(type()) << "\"";
