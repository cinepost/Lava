...
namespace parser {
    ...
    //    = (array | object) >> eps;
        //= lexeme[x3::lit("[") > backtracking(*(char_("a-zA-Z0-9\".[]=_~")), lit("]"))][assign_bgeo];

    x3::rule<class bgeo_, ast::Bgeo> const bgeo = "bgeo";
    //auto const bgeo_def = x3::raw[x3::lit("[") > backtracking(*(char_("a-zA-Z0-9\".[]=_~")), lit("]"))][assign_bgeo];
    //auto const bgeo_def = x3::lexeme[+(graph | char_('\n') ) % eol][assign_bgeo];
    auto const bgeo_def = skip(blank | char_("\n\t")) [x3::raw[backtracking(*graph, lit("]"))][assign_bgeo]];
    BOOST_SPIRIT_DEFINE(bgeo)
