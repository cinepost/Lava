...
def cmd_setenv(name, value):
    soho.indent()
    soho.print_('setenv', name, '=', cmd_quote(value))

def cmd_version(ver):
    soho.indent()
    soho.print_('cmd_version VER%s' % ver)
...
def cmd_defaults(file):
    soho.indent()
    soho.print_('cmd_defaults', repr(file))
...
def cmd_config(prop_name, soho_parm):
    ...
    soho.print_('cmd_config', prop_type, prop_name, prop_value_repr)
...
def cmd_detail(name, file):
    ...
    if file.find(' ') >= 0:
        file = '"%s"' % file
    soho.print_('cmd_detail', name, file)

def cmd_geometry(file):
    soho.indent()
    soho.print_('cmd_geometry', file)
...
def cmd_delete(type, name):
    soho.print_('cmd_delete', type, name)
...
def cmd_shop(shop, shaderstring, shoptype = soho.ShopTypeDefault):
    ...
    if len(shader[1]):
        soho.print_(shader[1])
    soho.indent()
    soho.print_('cmd_shop', shop, shader[0])
...
def cmd_shader(style, name, orgshader, shoptype = soho.ShopTypeDefault):
    ...
    if len(shader[1]):
        soho.print_(shader[1])
    soho.indent()
    soho.print_('cmd_property', style, name, shader[0])

def cmd_textblock(name, value, encoding=None):
    soho.indent()
    if encoding:
        soho.print_('cmd_textblock -e', encoding, name)
    else:
        soho.print_('cmd_textblock', name)
    soho.print_(value)
    soho.print_('cmd_endtext')

def cmd_erase_textblock(name):
    soho.indent()
    soho.print_('cmd_textblock -x', name)

def cmd_stylesheet(name):
    soho.indent()
    soho.print_('cmd_stylesheet', name)

def cmd_bundle(bundle, node_paths):
    soho.indent()
    soho.print_('cmd_bundlecreate', bundle)
    soho.addBundleDependency(bundle)
    start = 0
    step = 100
    while len(node_paths) > start:
        end = start + step
        soho.print_('cmd_bundleadd', bundle, " ".join(node_paths[start:end]))
        start = end
...
def cmd_declare_parm(style, name, soho_parm):
    ...
    soho.print_('cmd_declare', style, parm_type, name, parm_value_repr)
...
def cmd_commandline(options):
    soho.indent()
    soho.print_("cmd_cmdopt", options)
