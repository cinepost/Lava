...
class VopNodeNetworkAdapterBase(VopNodeAdapterAPI):
	...
	def generateIncludes(cls, vop_node_ctx):
		for adapter, context in vop_node_ctx.children():
			for generable in adapter.generateIncludes(context):
				assert isinstance(generable, code.Include )
				yield generable

	@classmethod
	def generateImports(cls, vop_node_ctx):
		for adapter, context in vop_node_ctx.children():
			for generable in adapter.generateImports(context):
				assert isinstance(generable, code.Import )
				yield generable