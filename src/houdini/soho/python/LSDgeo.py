...
def hashSopPathName(soppath):
    ...
    return "%s_%s" % (sopname, hashlib.sha256(safe_soppath.encode()).hexdigest()[:max_hexdigest_len])
...
def processShaderStrings(obj, strings, now, shadertype):
    for path in strings:
        if path:
            ...
            if pathkey not in theSavedShaders:
