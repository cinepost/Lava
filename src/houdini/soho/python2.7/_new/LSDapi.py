...
def cmd_edge(src_node_uuid, src_output_socket, dst_node_uuid, dst_input_socket):
    soho.indent()
    print('cmd_edge %s %s %s %s' % (src_node_uuid, src_output_socket, dst_node_uuid, dst_input_socket))

def cmd_socket(direction, data_type, name):
    soho.indent()
    print('cmd_socket %s %s %s' % (direction, data_type, name))
...
def cmd_shop(shop, shaderstring, shoptype = soho.ShopTypeDefault):
    ...
    if len(shader[1]):
        print(shader[1])
    soho.indent()
    print('cmd_shop %s' % (shop, shader[0]))
