...
class NodeManagerWrapper(NodeWrapperBase):
	...
	def _addDependencyEdge(self, from_node_wrapper, to_node_wrapper):
		self._dep_graph[from_node_wrapper.name()].append(to_node_wrapper.name())

	def _prepareDependencyGraph(self):
		self._dep_graph = defaultdict(list) #dictionary containing adjacency list

		for node_name in self._children:
			to_node_wrapper = self._children[node_name]
			for connection in to_node_wrapper.inputConnections():
				from_node_wrapper = connection.inputNodeWrapper()
				self._addDependencyEdge(from_node_wrapper, to_node_wrapper)


	# A recursive function used by topologicalSort
	def topologicalSortUtil(self, node_wrapper, visited, stack):
  
		# Mark the current node as visited.
		visited[node_wrapper.name()] = True
  
		# Recur for all the vertices adjacent to this vertex
		for node_name in self._dep_graph[node_wrapper.name()]:
			if visited[node_name] == False:
				self.topologicalSortUtil(self._children[node_name], visited, stack)
