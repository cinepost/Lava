...
static const std::string kWindowName = "Eagle";

Window::Window(int _x, int _y,int _width, int _height, int _ppp, GLenum pixelType, GLenum pixelFormat, GLenum texFormat) {
  m_name = kWindowName;
  m_x = _x;
  m_y = _y;
  m_width = _width;
...
void Window::updateImage(const void *_image) {
...
int Window::pollEvent(SDL_Event &_event) {
  makeCurrent();
  return SDL_PollEvent(&_event);
}

void Window::setScale(float _f) {
  m_scale=_f;
...
void Window::setPosition(float _x, float _y) {
  m_xPos=_x;
  m_yPos=_y;
  glUniform2f( m_translateUniform,m_xPos,m_yPos );
  draw();
...
void Window::ImGuiCleanup() const {
...
void Window::setRenderMode(RenderMode _m) {
  mRenderMode = _m;
...
void Window::setBackgroundMode(BackgroundMode _m) {
  mBackgroundMode = _m;
  switch(static_cast<int>(_m)) {
    case 0 : 
...
void Window::showFalseColors(bool m) { 
  mShowFalseColors = m;
  if (mShowFalseColors) {
...
void Window::setExposure(float _e) {
  m_exposure=_e;
